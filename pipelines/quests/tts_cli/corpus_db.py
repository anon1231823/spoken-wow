...
def fold_overrides(verbose=True):
    ...
    Deliberate and re-runnable rather than part of the import, for two reasons: it can only
    run once quest_line is seeded, and it is the one step here that changes what a line
    says. A second run writes nothing, because a line whose live version is already this
    text is left alone.
                ...
                cur.execute(
                    """insert into "quest_line"
                       ...
                       select "lineId", "variant", "lang", %s, true, 'edited',
                        ...
                        where "lineId" = %s and "variant" = %s and "lang" = %s
                          and "version" = %s""",
                    (version + 1, text, updated_by, line_id, variant, LANG, version),
                )
                folded += 1
    finally:
        conn.close()
