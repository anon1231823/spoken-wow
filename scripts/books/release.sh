#!/usr/bin/env bash
# Uploads the built books zips to CurseForge through the author API.
#
#   ./scripts/books/release.sh --dry-run   # say what would be sent, send nothing
#   ./scripts/books/release.sh             # both projects
...
for arg in "$@"; do
  case "$arg" in
    ...
    *) echo "error: unknown argument '$arg' (expected: books, audio, --dry-run)" >&2; exit 1;;
  esac
done
