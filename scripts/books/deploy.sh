...
#
# The addon ships for three clients and the only way to know it loads on all of them is to
# load it on each, so which client to install into is a variable rather than a
# constant. `forever` is the WoW: Forever beta, which Blizzard installs under the
# wow_classic_beta product directory -- confirmed by its own .flavor.info rather than
# guessed from the name. Installing targets one client; --status and --remove walk every client
...
# AddOns list does not show Spoken Books, fall back to --copy and re-run per change.
#
...
#
# The narration itself is NOT in the repository -- audio lives outside git -- so the pack
# folder here carries the lookup and the mp3s are whatever has been pulled into
# addons/SpokenBooksAudio/Sounds. --status says how many that is.
#
...
case "$mode" in
  --status)
    ...
    sounds="$REPO/addons/SpokenBooksAudio/Sounds"
    if [[ -d "$sounds" ]]; then
...
echo "  1. enable Spoken Books in the AddOns list at the character select screen"
echo "  2. /console scriptErrors 1     (surface Lua errors)"
echo "  3. /spb status                 (what is known, and what is narrated)"
echo "  4. open a book                 (autoplay reads it; /spb read starts it by hand)"
