...
export function NoteDialog({ line, onClose, onSave }: Props) {
  ...
  return (
    <dialog
      ref={dialog}
      onClose={onClose}
      className="fixed top-1/2 left-1/2 w-[32rem] max-w-[90vw] -translate-x-1/2 -translate-y-1/2 rounded-lg border border-border bg-panel p-4 text-fg backdrop:bg-black/60"
    >
      <form
        method="dialog"
        onSubmit={(event) => {
      ...
      >
        ...
        <p className="mb-3 text-xs text-faint">{line.zoneName}</p>
        ...
        <div className="mt-3 flex items-center justify-end gap-2">
          ...
          <button type="submit" className="rounded bg-accent px-3 py-1 text-bg hover:opacity-90">
            Save
          </button>
        </div>
      </form>
