 ...
 * repo has to boot without ceremony, and a deploy that forgot SPOKEN_SECRET_KEY must
...
// refused it.
const DEVELOPMENT_KEY = "spoken-development-key-000000000";
...
function masterKey(): Buffer {
  ...
  const raw = process.env.SPOKEN_SECRET_KEY;
  if (!raw) {
    if (process.env.NODE_ENV === "production") {
      throw new Error(
        "SPOKEN_SECRET_KEY is unset. It is the master key for stored ElevenLabs " +
  ...
  if (key.length !== 32) {
    throw new Error(
      `SPOKEN_SECRET_KEY must be 32 bytes of base64, got ${key.length}. ` +
        "Generate one with `openssl rand -base64 32`.",
    );
  }
