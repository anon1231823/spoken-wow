...
describe("versionsToPrune", () => {
  it("keeps everything until there are more than five takes", () => {
    expect(versionsToPrune([0])).toEqual([]);
    expect(versionsToPrune([0, 1, 2, 3, 4])).toEqual([]);
  });

  it("drops the oldest generated take, never version 0", () => {
    expect(versionsToPrune([0, 1, 2, 3, 4, 5])).toEqual([1]);
    expect(versionsToPrune([0, 1, 2, 3, 4, 5, 6])).toEqual([1, 2]);
  });

  it("keeps exactly KEEP_VERSIONS takes however many there are", () => {
    const versions = Array.from({ length: 20 }, (_, i) => i);
    const survivors = versions.filter((v) => !versionsToPrune(versions).includes(v));
    expect(survivors).toHaveLength(KEEP_VERSIONS);
    expect(survivors).toEqual([0, 16, 17, 18, 19]);
  });

  // A file first generated by this app has no inherited take, so nothing is pinned and the
  // window is a plain newest-five.
  it("falls back to the newest five when there is no version 0", () => {
    expect(versionsToPrune([1, 2, 3, 4, 5, 6])).toEqual([1]);
    expect(versionsToPrune([3, 4, 5])).toEqual([]);
  });

  it("survives gaps left by an earlier prune", () => {
    expect(versionsToPrune([0, 7, 8, 9, 10, 11])).toEqual([7]);
  });
});

describe("pruneVersionFiles", () => {
  it("removes only the takes it is given", async () => {
    fs.mkdirSync(historyDir(FILE), { recursive: true });
    for (const version of [0, 1, 2, 3]) fs.writeFileSync(versionPath(FILE, version), "x");

    await pruneVersionFiles(FILE, [1, 2]);

    expect(await versionsOnDisk(FILE)).toEqual([0, 3]);
  });

  it("is silent about a take that is already gone", async () => {
    await expect(pruneVersionFiles(FILE, [4])).resolves.toBeUndefined();
  });
});
