...
#   rollback.sh 20260802-1143-a1b2c3d    # a specific release
#   rollback.sh --list                   # what is available
#
# No CI, no network, no git: releases are directories and `current` is a symlink, so this
# is a local rename that takes about a second. It works when GitHub is down.
#
# What it does NOT do is move the schema. Migrations are forward-only, so rolling back
# code leaves the newer schema in place -- which is safe exactly as long as migrations
# stay additive. See deploy/bin/migrate.sh.
set -euo pipefail
