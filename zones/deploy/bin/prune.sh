#!/usr/bin/env bash
#
# Keep the newest N releases (default 5), never removing the current one.
#
# Each release is ~62 MB of standalone bundle plus a megabyte of Lua corpus. The audio
# store lives in shared/, not in a release, so pruning never touches the ~700 MB of mp3s
# or the archive of superseded takes beside them -- which is the entire reason those two
# are overridden out of the release tree in the first place. See deploy/README.md.
set -euo pipefail

ROOT=${ZONELORE_DEPLOY_ROOT:-/srv/zonelore}
RELEASES=$ROOT/releases
KEEP=${1:-5}

CURRENT=$(basename "$(readlink -f "$ROOT/current" 2>/dev/null || echo none)")

# The current release is skipped explicitly, in case it has fallen outside the newest N.
for r in $(ls -1 "$RELEASES" | sort -r | tail -n "+$((KEEP + 1))"); do
  if [ "$r" = "$CURRENT" ]; then
    echo "prune: skipping $r (current)"
    continue
  fi
  echo "prune: removing $r"
  rm -rf "${RELEASES:?}/$r"
done
