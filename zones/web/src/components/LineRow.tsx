...
export function LineRow({
  ...
  return (
    <>
        ...
        <td className="px-2 py-1.5 whitespace-nowrap">
          <div className="flex items-center gap-1.5">
            ...
            {/* The count is public; the bodies are not. So everyone sees how many open
                ...
                -- and only a triager gets a control that opens them. */}
            {line.feedbackOpen > 0 &&
              (canTriage ? (
                <button
                  type="button"
                  aria-expanded={expanded}
                  onClick={() => onToggleExpand(line)}
                  title={`${line.feedbackOpen} open report${line.feedbackOpen === 1 ? "" : "s"}`}
                  className="flex items-center gap-0.5 rounded bg-warn/15 px-1 text-xs text-warn"
                >
        ...
        <td className="p-0">
          ...
          >
            ...
            {line.translated === false ? (
              <span className="block text-faint italic">not translated</span>
            ) : (
      ...
      {/* Mounted only while expanded, so collapsing drops the fetched bodies and
          reopening asks again -- a report resolved from the /feedback page in another
          tab would otherwise still read as open here. */}
      {expanded && (
        <tr className="border-b border-border bg-panel/40">
          <td colSpan={colSpan} className="px-2 pb-2">
            <FeedbackPanel lineId={line.id} />
          </td>
        </tr>
      )}
    </>
