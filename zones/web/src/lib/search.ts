...
//
// Everything database-backed arrives in a SearchContext rather than being fetched
// here, which is the split ../wow-voiceover/web/src/lib/search.ts:82 makes and the
// reason this is testable with no database and no filesystem. 1353 rows filter in a
// linear chain per request; there is no index and at this size there does not need to
// be one.
//
// THE STATE DEFINITIONS ARE THE CLI's. `missing`, `stale` and `current` mean exactly
// what tools/voice/generate.mjs's --missing and --stale select, because they are
// computed the same way: stale is take.textHash !== entry.hash, where entry.hash is
// sha1 of the SPOKEN text. If these two ever disagree, the explorer is lying about
...
export type ResultLine = {
   ...
   *  one file per line -- so unlike ../wow-voiceover there is no positional row key. */
  id: string;
  kind: "zone" | "subzone";
  mapID: number;
  zoneName: string;
