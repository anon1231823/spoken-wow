...
//
// The addon half of that pair cannot import this file. Both `## Notes:` lines carry the
// same claim in their own words, and the CurseForge pages under `curseforge/` carry the
// long version -- and only those: nothing the client says out loud repeats it. They are
// meant to be edited in one sitting; the voice stops being "in redesign" everywhere on
// the same day.
//
// The `curseforge/` copies are the ones that can go stale in the other direction: they
// are pasted into a web form by hand, so the newer text may be the one on the site.
// `node tools/import-descriptions.mjs` pulls the live pages back down.
 ...
 *
 * Both halves are named, because both are unfinished and only one of them announces
 * itself. A bad line is heard immediately; a zone description quietly trimmed at the
