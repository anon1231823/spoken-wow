...
**Stage directions are narrated, and only by the web app.** Blizzard writes `<Advisor Belgrum opens the note.>` inside the NPC's own quest text. A *capitalised* bracketed span is a stage direction and is read by `narrator-male`; a *lowercase* one is a sound the NPC makes (`<hic>`, `<cough>`) and is left alone, so those lines stay unvoiceable rather than having a narrator say "hic". Capitalisation separates all 90 spans in the corpus with no exceptions — do not also require a full stop, which misclassifies `Motega shrugs his shoulder`. This means `web/src/lib/text-gate.ts` is no longer a mirror of `INVALID_CHARS` in `tts_cli/corpus.py`: the web app voices 55 lines the CLI still refuses. That is the second deliberate divergence between the two generators, alongside the pronunciation dictionary.
