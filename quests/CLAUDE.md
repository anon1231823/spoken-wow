...
**`make package-audio-hq` builds `VoiceOverReduxAudioHQ`** - every line in one folder at `ogg-q0-44k`, ~1.3 GB, never a CurseForge release (far over the ceiling) and distributed by hand. It is the one build using `MODULE_NAME`/`TITLE` to name a module outright rather than by pack suffix. `make package-audio-hq-split` cuts the same audio the four ways at full bandwidth (`NAME_SUFFIX=HQ`, ~300 MB each) - those *are* under the ceiling, and want projects of their own rather than a second file on the standard packs', since a manager installs a project's newest file and would move a player between qualities.
