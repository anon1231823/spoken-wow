...
describe("loading findings", () => {
  it("writes each finding once and links every line it touches", async () => {
    fresh();
    const report = await loadFindings([finding()]);
  ...
  it("does not write a name the lexicon already answers", async () => {
    fresh();
    // Seeded by migration 0008, and the whole reason coverage is decided here and not in
    // the scan: this row is edited from /lexicon and the scan cannot see it.
    const report = await loadFindings([finding({ item: "Gnomeregan", grapheme: "gnomeregan" })]);
  ...
  it("drops a line that stopped saying the thing", async () => {
    fresh();
    await loadFindings([finding()]);
    await loadFindings([finding({ lineIds: [lineA] })]);
  ...
  it("keeps a finding the newest scan did not see, but marks it undetected", async () => {
    fresh();
    await loadFindings([finding()]);
    await loadFindings([finding({ item: "Splintertree", grapheme: "splintertree" })]);
...
describe("issuesByLine", () => {
  ...
  it("stops marking a line once the scan no longer detects the finding", async () => {
    fresh();
    await loadFindings([finding()]);
    await loadFindings([finding({ item: "Splintertree", grapheme: "splintertree", lineIds: [] })]);
