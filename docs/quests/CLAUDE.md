...
**`make package-audio-complete` builds `VoiceOverReduxAudioHQ`** - every line in one folder at `ogg-q0-44k`, ~1.3 GB, never a CurseForge release (far over the ceiling) and hosted on the site as `/downloads/VoiceOverReduxAudioHQ-latest.zip`, which is why that folder name is fixed. It is the one build using `MODULE_NAME`/`TITLE` to name a module outright rather than by pack suffix. **The shipping packs are `VoiceOverReduxHQAudio*`** (folder) / "Spoken Quests Audio: X" (title), `ogg-q0-44k`, ~300 MB a pack, five CurseForge projects. A second set at `ogg-q-1-22k` shipped under `VoiceOverReduxAudio*` until the rename and is retired: those projects stay published and nothing uploads to them. `MODULE` and `TITLE_FAMILY` in `scripts/quests/package-audio.sh` are what name a set of packs; `tts_cli/factions.py:pack_title` composes a title from a family and a pack label. A set of packs per project rather than two files on one, since a manager installs a project's newest file and would move a player between formats - the reason there were two, and the reason the retired projects are not reused. **The folder names do not change**: `DataModules:EnumerateAddons` keys `presentModules` by folder name, so a rename is a re-download and a player told they are missing what they have.
